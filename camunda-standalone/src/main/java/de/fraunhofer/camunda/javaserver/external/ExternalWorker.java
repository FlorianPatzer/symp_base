...
public class ExternalWorker implements ExternalTaskHandler {
    ...
    public void completeTaskWithFile(File file, ExternalTaskService externalTaskService, ExternalTask externalTask) {
        ...
        variables.put(this.currentExternalTask.getActivityId(), typedFileValue);
        localVars.put("local", "this is local " + (int) Math.floor(Math.random() * 10));
        externalTaskService.complete(externalTask, variables, localVars);
    }
