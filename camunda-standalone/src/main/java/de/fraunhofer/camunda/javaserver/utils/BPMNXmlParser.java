...
public class BPMNXmlParser {
    ...
    public Collection<String> getJoinableOnts(String activityId) {
        ...
        /*Predicate<FlowNode> gatewayType = s -> s.getElementType()
                                                .getBaseType()
                                                .getTypeName()
                                                .equals("gateway");*/
        
        /*while (sources.stream()
                      .anyMatch(gatewayType)) {
            List<FlowNode> expanded = sources.stream()
                                             .filter(gatewayType)
                                             .flatMap(s -> s.getIncoming()
                                                        .stream()
                                                        .map(SequenceFlow::getSource))
                                             .collect(Collectors.toList());

            List<FlowNode> returnVal = sources.stream()
                                              .filter(gatewayType.negate())
                                              .collect(Collectors.toList());
            returnVal.addAll(expanded);
            sources = returnVal;
        }*/
